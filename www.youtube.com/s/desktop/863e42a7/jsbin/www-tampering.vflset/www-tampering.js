...
*/
    ...
    u("Symbol", function (a) {
        function b(l) { if (this instanceof b) throw new TypeError("Symbol is not a constructor"); return new c(f + (l || "") + "_" + h++, l) }
    ...
    u("WeakMap", function (a) {
        function b(d) { this.g = (e += Math.random() + 1).toString(); if (d) { d = w(d); for (var g; !(g = d.next()).done;)g = g.value, this.set(g[0], g[1]) } }
        function c() { }
    ...
    function z(a, b) {
        a instanceof String && (a += ""); var c = 0, f = !1, h = { next: function () { if (!f && c < a.length) { var l = c++; return { value: b(l, a[l]), done: !1 } } f = !0; return { done: !0, value: void 0 } } };
    ...
    u("Set", function (a) {
        function b(c) { this.g = new Map; if (c) { c = w(c); for (var f; !(f = c.next()).done;)this.add(f.value) } this.size = this.g.size }
    ...
    function O(a) {
        var b = a.split("."); a = b[b.length - 1]; b = b.reduce(function (c, f) { return c && c[f] }, window);
    ...
    function S() {
        var a = new Set;[].concat(x(document.querySelectorAll("script"))).forEach(function (b) { b.src && !Q(b.src) && a.add(b.src) });
    ...
    function T() {
        var a = new Set;[].concat(x(document.querySelectorAll("script"))).forEach(function (b) { b.src && (b = R(b.src)) && a.add(b) });
        return [].concat(x(a)).sort()
    }
    ; B("ytbin.polymer.shared.lib.tampering.info", function () { var a = S(), b = N(), c = T(), f = []; c.length && f.push("extensions", c); a.length && f.push("suspiciousIncludes", a); b.length && f.push("suspiciousApis", b); return f.length ? f : null });
}).call(this);
